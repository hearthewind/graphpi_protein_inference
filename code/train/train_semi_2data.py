...
from train.util import save_torch_algo, get_dataset, get_model, iprg_converter, get_node_features
...
def main():
    ...
    for dataset in DATA_LIST:
        if dataset in TEST_DATA:
            data = get_dataset(dataset, train=True, prior=args.prior, prior_offset=args.prior_offset, protein_label_type="decoy", pretrain_data_name="epifany", loss_type=args.loss_type)
        else:
            data = get_dataset(dataset, train=True, prior=args.prior, prior_offset=args.prior_offset, protein_label_type="decoy_sampling", pretrain_data_name="epifany", loss_type=args.loss_type)
    ...
    for dataset in DATA_LIST:
        if dataset in TEST_DATA:
            data = get_dataset(dataset, train=True, prior=args.prior, prior_offset=args.prior_offset, protein_label_type="decoy", pretrain_data_name="fido", loss_type=args.loss_type)
        else:
            data = get_dataset(dataset, train=True, prior=args.prior, prior_offset=args.prior_offset, protein_label_type="decoy_sampling", pretrain_data_name="fido", loss_type=args.loss_type)
        num_valid = int(len(data.proteins)*0.2)
        train_node_idx = np.array(data.proteins)
        np.random.shuffle(train_node_idx)
        valid_proteins = torch.LongTensor(train_node_idx[:num_valid])
    ...
    for epoch in range(args.epochs):
        ...
        if best_valid_score < valid_score:
            ...
            save_torch_algo(models, f"pretrain_small_noniso_{args.loss_type}_{args.pretrain_data_name}_prior-{args.prior}_offset-{args.prior_offset}")

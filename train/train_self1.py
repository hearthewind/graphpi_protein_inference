...
def parse_args():
    ...
    parser.add_argument('--save_result_dir', type=str, default="self_best")
...
def store_result(args, test_datasets, device, models, test_fdr=0.05):
    if args.best:
        return store_result_best(args, test_datasets, device, test_fdr=test_fdr)
    else:
        return store_results_onemodel(models, test_datasets, device)


def store_result_best(args, test_datasets, device, test_fdr=0.05):
    ...
    final_result_dict = {}
    final_result_dict[0] = result_dict
    ...
    for dataset in test_datasets:
        best_model = 0
        best_tp_decoy = 0
        for model_no in range(args.rounds + 1):
            result = final_result_dict[model_no][dataset.dataset]
            result = {iprg_converter(protein, dataset): score for protein, score in result.items()}
            tp_decoy = len(get_proteins_by_fdr(result, fdr=test_fdr))
            if tp_decoy > best_tp_decoy:
                best_model = model_no
                best_tp_decoy = tp_decoy
        best_result_dict[dataset.dataset] = final_result_dict[best_model][dataset.dataset]

    len_true_proteins = {}
    for dataset in test_datasets:
        result = best_result_dict[dataset.dataset]
        reverse_id_map = {value: key for key, value in dataset.protein_map.items()}
    ...
    for dataset, data in best_result_dict.items():
        if "2016TS" in dataset:
            ...
            dataset_prefix, dataset_type = dataset.split("_")
            data_path = os.path.join(PROJECT_ROOT_DIR, PROJECT_DATA_DIR, f"{dataset_prefix.lower()}",
                                     f"mymodel_result",
                                     f"result_{dataset_type}.pkl")
        else:
            data_path = os.path.join(PROJECT_ROOT_DIR, PROJECT_DATA_DIR, f"{dataset}", "mymodel_result",
                                     "result.pkl")
    ...
    return best_result_dict, len_true_proteins
...
@torch.no_grad()
def test(models, data_samplers, device, loss_type="cross_entropy"):

...
def parse_args():
    ...
    parser.add_argument('--edge_mode', type=int, default=8)  # 0: use it as weight; 1: as input to mlp
    parser.add_argument('--gnn_activation', type=str, default='relu')
    parser.add_argument('--dropout', type=float, default=0.)
    parser.add_argument('--use_deeppep', type=bool, default=True)
...
def main():
    ...
    for dataset in DATA_LIST:
        ...
        if args.batch_size <= 0:
            ...
            train_sampler = get_sampler(data, train_proteins, args.batch_size, fan_out=[20] + [10] * (args.num_gnn_layers - 1), rebalance=False)
            valid_sampler = get_sampler(data, valid_proteins, args.batch_size, fan_out=[20] + [10] * (args.num_gnn_layers - 1), rebalance=False)
            # train_sampler = get_sampler(data, train_proteins, args.batch_size, fan_out=[25] * args.num_gnn_layers, rebalance=False)
            # valid_sampler = get_sampler(data, valid_proteins, args.batch_size, fan_out=[25] * args.num_gnn_layers, rebalance=False)
    ...
    for epoch in range(args.epochs):
        ...
        model_name = f"pretrain_deeppep_{args.loss_type}_{args.pretrain_data_name}_{args.protein_label_type}_prior-{args.prior}_offset-{args.prior_offset}"

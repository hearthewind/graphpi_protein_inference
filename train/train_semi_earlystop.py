...
def parse_args():
    ...
    parser.add_argument('--save_result_dir', type=str, default="self_earlystop")
...
def main():
    ...
    best_valid_tp_decoy = defaultdict(int)
    best_test_tp = defaultdict(int)
    ...
    for epoch in range(args.epochs):
        ...
        if args.batch_size <= 0:
            ...
            len_tp_decoy, _ = evaluate_hetero_decoy(models, test_datasets, device)
            len_true_proteins, _ = evaluate_hetero(models, test_datasets, device)
        else:
            ...
            len_tp_decoy, _ = evaluate_hetero_batch_decoy(models, test_samplers, device)
        ...
        for dataset in test_datasets:
            if len_tp_decoy[dataset.dataset] > best_valid_tp_decoy[dataset.dataset]:
                best_test_tp[dataset.dataset] = len_true_proteins[dataset.dataset]
                best_valid_tp_decoy[dataset.dataset] = len_tp_decoy[dataset.dataset]

                save_torch_algo(models, f"pretrain_{dataset.dataset}_{args.loss_type}_{args.pretrain_data_name}_{args.protein_label_type}_prior-{args.prior}_offset-{args.prior_offset}")
                print(f'Saved model for data {dataset.dataset}, with tp decoy {len_tp_decoy[dataset.dataset]}, best tp contaminate {best_test_tp[dataset.dataset]}')

    store_results(models, test_datasets, device, args)
    get_fdr_vs_TP_graphs()


def store_results(models, test_datasets, device, args):
    final_len_true_proteins = {}
    final_result_dict = {}

    for dataset in test_datasets:
        model_name = f"pretrain_{dataset.dataset}_{args.loss_type}_{args.pretrain_data_name}_{args.protein_label_type}_prior-{args.prior}_offset-{args.prior_offset}"
        models = load_torch_algo(model_name, models, device)

        len_true_proteins, result_dict = evaluate_hetero(models, [dataset], device)
        final_result_dict[dataset.dataset] = result_dict[dataset.dataset]
        final_len_true_proteins[dataset.dataset] = len_true_proteins[dataset.dataset]

    len_true_proteins = final_len_true_proteins
...
@torch.no_grad()
def evaluate_hetero_decoy(model, datasets, device, test_fdr=0.05):
    print(f'Evaluating ...')
    ...
    for dataset in datasets:
        ...
        true_proteins = get_proteins_by_fdr(result, fdr=test_fdr)
...
@torch.no_grad()
def evaluate_hetero_batch_decoy(model, data_samplers, device, test_fdr=0.05):
    print(f'Evaluating ...')
    model.eval()

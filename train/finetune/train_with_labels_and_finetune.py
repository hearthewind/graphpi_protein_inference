...
from train.util import save_torch_algo, get_model, get_dataset, get_node_features, iprg_converter
...
def main():
    ...
    for dataset in DATA_LIST:
        if dataset.startswith("PXD"):
            ...
            data_samplers_train.append((data, train_proteins, valid_proteins))
            gc.collect()
        else:
            data = get_dataset(dataset, train=True, prior=args.prior, prior_offset=args.prior_offset, protein_label_type="groundtruth", pretrain_data_name=args.pretrain_data_name, loss_type=args.loss_type)
            num_valid = int(len(data.proteins)*0.2)
            train_node_idx = np.array(data.proteins)
            np.random.shuffle(train_node_idx)
            valid_proteins = torch.LongTensor(train_node_idx[:num_valid])
            train_proteins = torch.LongTensor(train_node_idx[num_valid:])
            data_samplers_finetune.append((data, train_proteins, valid_proteins))
            gc.collect()
    ...
    model_dir = f"finetune_small_noniso_{args.loss_type}_{args.pretrain_data_name}_prior-{args.prior}_offset-{args.prior_offset}"

    best_valid_score = 0
    for epoch in range(args.epochs):
        train_loss = train(models, data_samplers_train, device, optimizer, epoch, args.loss_type)
        valid_loss, valid_score = test(models, data_samplers_train, device, args.loss_type)
    ...
    best_valid_score = 0
    for epoch in range(args.epochs):
        train_loss = train(models, data_samplers_finetune, device, optimizer, epoch, args.loss_type)
        valid_loss, valid_score = test(models, data_samplers_finetune, device, args.loss_type)

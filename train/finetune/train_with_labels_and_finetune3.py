...
def main():
    ...
    if args.use_pretrain:
        ...
        fine_tune_by_dataset(args, models, "iPRG2016_A", device)
        fine_tune_by_dataset(args, models, "iPRG2016_B", device)
        fine_tune_by_dataset(args, models, "ups2", device)
        return
...
def fine_tune_by_dataset(args, models, dataset, device):

    data = get_dataset(dataset, train=True, prior=args.prior, prior_offset=args.prior_offset, protein_label_type="decoy", pretrain_data_name=args.pretrain_data_name, loss_type=args.loss_type, use_deeppep=args.use_deeppep)

    len_proteins, protein_scores = evaluate_hetero(models, [data], device)
    print(len_proteins)

    # scores_dict = sorted(dict(zip(data.proteins, protein_scores)).items(), key=lambda x: x[1], reverse=True)
    # data.pos_proteins = [protein for protein, score in scores_dict[:int(len(scores_dict) * 0.1)]]
    # data.neg_proteins = [protein for protein, score in scores_dict[int(len(scores_dict) * 0.5):]]
    # data.proteins = data.pos_proteins + data.neg_proteins
    # data.update_label()
    ...
    for model_num, seed_batch in enumerate(seed_batchs):
        ...
        for epoch in range(50):
            ...
            train_loss = train_hetero(models, [(data, train_index, train_index)], device, optimizer, args.loss_type)

            len_true_proteins, probs = evaluate_hetero(models, [data], device)
            best_probs = probs

...
def parse_args():
    ...
    parser.add_argument('--save_result_dir', type=str, default="target_decoy")
...
def main():
    ...
    for dataset in DATA_LIST:
        ...
        train_node_indexes.append(train_node_idx)
        train_num_valids.append(num_valid)

    for fold in range(5):

        data_samplers = []
        for i, data in enumerate(train_datasets):
            train_node_idx = train_node_indexes[i]
            num_valid = train_num_valids[i]

            stop1 = fold * num_valid
            stop2 = (fold + 1) * num_valid
            valid_proteins = torch.LongTensor(train_node_idx[stop1:stop2])
            train_proteins = torch.LongTensor(np.concatenate([train_node_idx[:stop1], train_node_idx[stop2:]]))
        ...
        valid_datasets = [dataset for dataset in test_datasets]
        ...
        model_name = f"{args.save_result_dir}/fold{fold}_{args.loss_type}_{args.pretrain_data_name}_{args.protein_label_type}_prior-{args.prior}_offset-{args.prior_offset}"
    ...
    result_dict, len_true_proteins = store_results(args, test_datasets, device, test_fdr=0.05)
    num_layers = len(args.model_types.split("_"))
    get_fdr_vs_TP_graphs(save_dir=args.save_result_dir, result_dict=result_dict,
                         image_post_fix=f"{num_layers}_{args.node_hidden_dim}_{args.epochs}_{args.lr}")


@torch.no_grad()
def store_results(args, test_datasets, device, test_fdr=0.05):
    ...
    for fold in range(5):
        ...
        _, result_dict = evaluate_hetero(models, test_datasets, device)
    ...
    for dataset, data in avg_result_dict.items():
        if "2016" in dataset:
            ...
            data_path = os.path.join(PROJECT_ROOT_DIR, PROJECT_DATA_DIR, f"{dataset_prefix.lower()}", f"mymodel_td_result",
                                     f"result_{dataset_type}.pkl")
        else:
            data_path = os.path.join(PROJECT_ROOT_DIR, PROJECT_DATA_DIR, f"{dataset}", "mymodel_td_result", "result.pkl")
...
@torch.no_grad()
def store_results_2(args, test_datasets, device, train_num_valids, train_node_indexes, test_fdr=0.05):
    models = get_model(args, device, test_datasets[0])
    models.eval()
    ...
    for fold in range(5):
        ...
        for i, (dataset_name, data_result) in result_dict.items():
            id_map = test_datasets[i].id_map
            num_valid = train_num_valids[i]
            train_node_idx = train_node_indexes[i]
            ...
            for key, value in data_result.items():
                protein_id = id_map[key]
                if protein_id in valid_proteins:
                    final_result_dict[dataset_name][key] = value

    avg_result_dict = final_result_dict
